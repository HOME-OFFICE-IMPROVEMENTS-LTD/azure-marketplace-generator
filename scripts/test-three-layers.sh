...
# Paths
GENERATOR_PATH="${1:-../azure-marketplace-generator}"
PLUGIN_PATH="${2:-../azmp-plugin-vm}"
...
npm test || { echo -e "${RED}✗ Plugin tests failed${NC}"; exit 1; }
PASSING_TESTS=$(npm test 2>&1 | grep -o '[0-9]* passing' | head -1)
